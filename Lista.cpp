...
void Lista::insere_fim(int i){ //Inserir dado i no final da Lista
	if(!vazia()){
...
bool Lista::retira_inicio(){ //Remove 1o no dado retornando true caso encontre
	if(!vazia()){
		if(this->cabeca->prox == NULL){ //Somente 1 nó dado, vai esvaziar a lista
			Lista::libera();
		}else{
			no_dado* p = this->cabeca;
...
bool Lista::retira_fim(){ //Remove 1o no dado retornando true caso encontre
	if(!vazia()){
		if(this->cabeca->prox == NULL){ //Somente 1 nó dado, vai esvaziar a lista
			Lista::libera();
		}else{
...
void Lista::imprime_inicio(){ //Imprime a lista do início para o fim
	if(!isEmpty(this->cabeca)){
		cout << "Primeiro item da lista : " << this->cabeca->info << endl;
	} else {
		cout << "Lista é vazia!" << endl;
	}
}

void Lista::imprime_fim(){ //Imprime a lista do início para o fim
	if(!isEmpty(this->cabeca)){
		no_dado* p = this->cabeca; 
		while (p->prox != NULL) {
			p = p->prox;
		}
		cout << "Último item da lista : " << p->info << endl;
	} else {
		cout << "Lista é vazia!" << endl;
	}
}
